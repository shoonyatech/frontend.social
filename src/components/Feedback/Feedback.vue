  ...
  methods: {
    async sendFeedback() {
      try {
        ...
        this.isActive = false;
        this.feedback = '';
        eventBus.$emit('show-toast', {body: messages.feedback.success, title: messages.generic.success});
      } catch(ex) {
        eventBus.$emit('show-toast', {body: messages.feedback.error, title: messages.generic.error, type: ToastType.ERROR});
      }
    }
  }
};
